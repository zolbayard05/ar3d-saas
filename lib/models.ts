...
// Every ModelCard box is this ratio, regardless of the underlying photo's
// or render's own shape (2026-09-03 product decision: uniform card size
// across the feed and library, replacing the old masonry layout where each
// card's height came from its own image's aspect ratio). MasonryGrid.tsx no
// longer needs to estimate per-card height for column-balancing — every
// card is identical, so a plain CSS grid places them correctly by
// construction.
export const MODEL_CARD_ASPECT_RATIO = 1;
